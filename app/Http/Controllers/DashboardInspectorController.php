...
class DashboardInspectorController extends Controller
{
    public function inspector_dashboard()
    {
		...
		$user_locations = $data['userdata']->get_user_location->pluck('location_id')->toArray();
		// dd(implode(',', $user_locations));
		$taskListIds = Task_lists::whereIn('location_id', [implode(',', $user_locations)])->pluck('id');
		$categoryIds = Task_list_subcategories::whereIn('task_list_id', $taskListIds)->pluck('task_list_category_id');
		$subcategoriesTaskIds = Task_list_subcategories::whereIn('task_list_category_id', $categoryIds)->pluck('task_list_id');
		...
		$data['correctiveApprovedCount'] = DB::table(DB::raw("({$baseChecklistQuery->toSql()}) as combined"))
			->mergeBindings($baseChecklistQuery)->count();
		// dd($correctiveApprovedCount);
	...
	public function checklist_next_question(Request $request)
	{
		...
		*/
		...
		if($task_type_data->task_type == 1)
		{
			//after change logic for selected checklist and selected subchecklist when task created 17-10-2025
			$checklistIds = Task_categories_checklist_subchecklist::where('category_id', $category_id)
			->where('task_list_id', $task_id)
			->orderBy('id', 'asc') // order as inserted
			->pluck('checklist_id')
			->unique()
			->values();

			$nextChecklistId = null;
			foreach ($checklistIds as $index => $checkId) {
				if ($checkId == $current_question_id && isset($checklistIds[$index + 1])) {
					$nextChecklistId = $checklistIds[$index + 1];
					break;
				}
			}
			
			$nextQuestionExists = $nextChecklistId !='' ? true : false;
		}
		else
		{
			// previous work start first day
				...
				if($approveStatus == 0)
				{
					if ($request->has('checklist_file')) {
						...
						foreach($checklist_files as $file)
						{
							...
							$model->task_list_checklist_id = $task_list_checklist_id ?? null;
				...
				foreach ($rejectTextsMultiple as $subChecklistId => $text) {
					...
					if($text['approve_status'] !='')
					{
						...
						if($hasid)
						{
							...
							if($text['approve_status']=='1')
							{
								$subchklistFiles = Task_list_subchecklist_rejected_files::where('task_list_checklist_id', $hasid)->get();
								if($subchklistFiles->isNotEmpty()){
						...
						// direct upload the subchecklist image to folder and table
							if ($request->has('subchecklist_file')) {
								foreach ($request->file('subchecklist_file') as $subchecklist_id => $files) {
									if($subchecklist_id == $subChecklistId)
									{
										foreach ($files as $file) {
											...
											$model->task_list_checklist_id = $task_list_subchecklist_id ?? null;
		...
		if($nextQuestionExists)
		{
			...
			if($task_type_data->task_type == 1)
			{
				...
				if($nextChecklistId)
				{
					$subIds = Task_categories_checklist_subchecklist::where('category_id', $category_id)->where('task_list_id', $task_id)->where('checklist_id', $nextChecklistId)->orderBy('id', 'ASC')->pluck('subchecklist_id')->toArray();
					
					$nextQuestion = Checklist::with(['get_subchecklist' => function ($query) use ($subIds) {
							$query->whereIn('id', $subIds);
						}
					, 'get_category'])->where('id', $nextChecklistId)->first();
				}
			}
			else
			{
	...
	public function checklist_previous_question(Request $request)
	{
		...
		if($nextQuestionExists)
		{
			...
			//$task_list_checklist_id = $iffetch ? $iffetch->checklist_id : null;
	...
	public function get_checklist_page(Request $request)
	{
		...
		if($task_type_data->task_type == 1)
		{
			...
			if(!empty($directEdit))
			{
				$subIds = Task_categories_checklist_subchecklist::where('category_id', $category_id)->where('task_list_id', $task_id)->where('checklist_id', $current_question_id)->orderBy('id', 'ASC')->pluck('subchecklist_id')->toArray();
			...
			{
				$chklstId = Task_categories_checklist_subchecklist::where('category_id', $category_id)->where('task_list_id', $task_id)->orderBy('id', 'ASC')->first()->checklist_id;
				
				$subIds = Task_categories_checklist_subchecklist::where('category_id', $category_id)->where('task_list_id', $task_id)->where('checklist_id', $chklstId)->orderBy('id', 'ASC')->pluck('subchecklist_id')->toArray();
	...
	public function submit_lo_corrective_action(Request $request)
	{
		...
		$model->tab_no = $request->lo_direct_approve == 'true' ? 2 : 3;
	...
	public function submit_inspector_status(Request $request)
	{
		...
		// update the date of tasklist_checklist and tasklist_subchecklist table
		if(!empty($checklist_id) && !empty($subchecklist_id))
		{
			Task_list_subchecklists::where('task_list_id', $task_list_id)->where('task_list_checklist_id', $checklist_id)->where('subchecklist_id', $subchecklist_id)->update(['updated_at' => date('Y-m-d h:i:s')]);
		}
		
		if(!empty($checklist_id) && empty($subchecklist_id))
		{
			Task_list_checklists::where('task_list_id', $task_list_id)->where('checklist_id', $checklist_id)->update(['updated_at' => date('Y-m-d h:i:s')]);
		}
		//--------------------------------------------
	...
	public function ins_load_more_needed_data(Request $request)
	{
		...
		$excludedChecklistPairs = Task_list_corrective_action::whereNotIn('rejected_repeated', [0])
			->whereIn('task_list_id', $taskListIds)
			->where('lo_id', auth()->user()->id) // 26-09-2025
			->orWhereIn('lo_direct_approve', [0, 1]) // 26-09-2025
			->where(function ($q) {
				$q->where(function ($q) {
					$q->where('inspector_action', 0)->where('los_action', 0);
				})->orWhere(function ($q) {
	...
	public function lo_transfer_location(Request $request)
	{
		...
		$isCorrectiveAction = $query->first();
