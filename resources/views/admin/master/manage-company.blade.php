	...
	<div class="content container-fluid">
		...
		<div class="page-header">
			<div class="row align-items-center">
				<div class="col-md-4">
					<h3 class="page-title">{{ __('manage_company') }}</h3>
					<ul class="breadcrumb">
						<li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('dashboard') }}</a></li>
						<li class="breadcrumb-item active">{{ __('manage_company') }}</li>
					</ul>
				</div>
				<div class="col-md-8 float-end ms-auto">
					<div class="d-flex title-head">
						<div class="view-icons">
							<a href="{{ route('admin.manage-company') }}" class="grid-view btn btn-link"><i class="las la-redo-alt"></i></a>
						...
						<a href="#" class="btn add-btn add_company" data-bs-toggle="modal" data-bs-target="#add_company"><i class="la la-plus-circle"></i> {{ __('add') }}</a>
					</div>
				</div>
			</div>
		</div>
		...
		<form name="search-frm" method="post" action="{{ route('admin.manage-company')}}" id="search-company-name">
		@csrf
			<div class="row filter-row">
				<div class="col-xl-3">  
					 <div class="input-block">
						 <input type="text" class="form-control" name="search_name" placeholder="{{ __('company_name')}}" value="{{ old('search_name', request('search_name'))}}">
					 </div>
				</div>
				<div class="col-xl-3">  
					 <div class="input-block">
			...
			<div class="col-lg-6 mb-2">
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						<li><a class="dropdown-item" onclick="change_multi_status('1','Manage_company','{{url('admin/change-multi-status')}}')">Active</a></li>
						<li><a class="dropdown-item" onclick="change_multi_status('0','Manage_company','{{url('admin/change-multi-status')}}')">Inactive</a></li>
						<li><a class="dropdown-item" onclick="delete_multi_data('Manage_company','{{url('admin/delete-multi-data')}}')">Delete</a></li>
					</ul>
				</div>
			</div>
		@endif
					...
					<table class="table table-striped custom-table datatable">
						...
						@foreach($manage_company as $val)
							<tr>
								...
								@endif
								<td class="contact-details" data-url="">{{ $val->company_name ?? ''}}</td>
								...
								@if($val->status ==1)
									<div class="dropdown action-label">
										...
										<div class="dropdown-menu dropdown-menu-right">
											<a class="dropdown-item update-status" href="javascript:void(0);" data-id="{{ $val->id }}" data-url="{{ route('admin.company_name_update_status') }}"><i class="fa-regular fa-circle-dot text-success"></i> {{ __('active') }}</a>
											<a class="dropdown-item update-status" href="javascript:void(0);" data-id="{{ $val->id }}" data-url="{{ route('admin.company_name_update_status') }}"><i class="fa-regular fa-circle-dot text-danger"></i> {{ __('inactive') }}</a>
										</div>
									</div>
								 @else
									<div class="dropdown action-label">
								...
								<td class="text-end">
									<div class="dropdown dropdown-action">
										...
										<div class="dropdown-menu dropdown-menu-right">
											<a class="dropdown-item edit-company-name" href="javascript:void(0);" data-id="{{ $val->id ??''}}" data-url="{{ route('admin.edit-company-name') }}"><i class="fa-solid fa-pencil m-r-5"></i> {{ __('edit') }}</a>
											{{--<a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#edit_contact"><i class="fa-solid fa-pencil m-r-5"></i> Edit</a>--}}
											<a class="dropdown-item delete-company-name text-danger" href="javascript:void(0);" data-id="{{ $val->id ?? '' }}" data-url="{{ route('admin.getDeleteCompanyName') }}"><i class="fa-regular fa-trash-can m-r-5"></i> {{ __('delete') }}</a>
...
@include('admin.modal.manage-company-modal')
@endsection 
@section('scripts')
@include('_includes.footer')
<script src="{{ url('admin-assets/js/page/manage_company.js') }}"></script>
