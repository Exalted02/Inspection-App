			...
			<div class="row align-items-center">
				...
				<div class="col-md-8 float-end ms-auto">
					<div class="d-flex title-head">
						...
						<a href="#" class="btn add-btn add_location_owner" data-bs-toggle="modal" data-bs-target="#add_location_owner"><i class="la la-plus-circle"></i> {{ __('add') }}</a>
					</div>
				</div>
			</div>
		</div>
							...
							<tr>
								...
								@if($val->status ==1)
									<div class="dropdown action-label">
										...
										<div class="dropdown-menu dropdown-menu-right">
											<a class="dropdown-item update-status" href="javascript:void(0);" data-id="{{ $val->id }}" data-url="{{ route('admin.location_owner_update_status') }}"><i class="fa-regular fa-circle-dot text-success"></i> {{ __('active') }}</a>
											<a class="dropdown-item update-status" href="javascript:void(0);" data-id="{{ $val->id }}" data-url="{{ route('admin.location_owner_update_status') }}"><i class="fa-regular fa-circle-dot text-danger"></i> {{ __('inactive') }}</a>
										</div>
									</div>
								 @else
									<div class="dropdown action-label">
								...
								<td class="text-end">
									<div class="dropdown dropdown-action">
										...
										<div class="dropdown-menu dropdown-menu-right">
											<a class="dropdown-item edit-location-owner" href="javascript:void(0);" data-id="{{ $val->id ??''}}" data-url="{{ route('admin.edit-location-owner') }}"><i class="fa-solid fa-pencil m-r-5"></i> {{ __('edit') }}</a>
											<a class="dropdown-item delete-location-owner-name" href="javascript:void(0);" data-id="{{ $val->id ?? '' }}" data-url="{{ route('admin.getDeletelocationowner') }}"><i class="fa-regular fa-trash-can m-r-5"></i> {{ __('delete') }}</a>
										</div>
									</div>
								</td>
							</tr>
...
@include('admin.modal.location-owner-modal')
@endsection 
@section('scripts')
...
$(document).ready(function() {
	...
	$(document).on('click',".reset-button", function(){
		window.location.href = "{{ route('admin.location-owner')}}";
	});
	
	const translations = {
        addlocation: @json(__('add_location_owner')),
    };
