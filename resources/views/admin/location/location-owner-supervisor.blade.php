...
@section('content')
@php 
//echo "<pre>";print_r($location_owner);die;
@endphp
<!-- Page Wrapper -->
	...
	<div class="content container-fluid">
		...
		<div class="page-header">
			<div class="row align-items-center">
				<div class="col-md-4">
					<h3 class="page-title">{{ __('location_owner') }}</h3>
					<ul class="breadcrumb">
						<li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('dashboard') }}</a></li>
						<li class="breadcrumb-item active">{{ __('location_owner') }}</li>
					</ul>
				</div>
				<div class="col-md-8 float-end ms-auto">
					<div class="d-flex title-head">
						<div class="view-icons">
							<a href="{{ route('admin.location-owner') }}" class="grid-view btn btn-link"><i class="las la-redo-alt"></i></a>
						...
						<a href="#" class="btn add-btn add_location_owner_supervisor" data-bs-toggle="modal" data-bs-target="#add_location_owner_supervisor"><i class="la la-plus-circle"></i> {{ __('add') }}</a>
					</div>
				</div>
			</div>
		</div>
		...
		<form name="search-frm" method="post" action="{{ route('admin.location-owner-supervisor')}}" id="search-location-owner-supervisor-frm">
		@csrf
			<div class="row filter-row">
				<div class="col-xl-3">  
					 <div class="input-block">
						 <input type="text" class="form-control" name="search_name" placeholder="{{ __('location_owner_name')}}" value="{{ old('search_name', request('search_name'))}}">
					 </div>
				</div>
				<div class="col-xl-3">  
					 <div class="input-block">
		 ...
		 <!-- /Search Filter -->
		 <div class="row">
		 @if($locationownersupervisor->count() > 0)
				...
				<div class="table-responsive">
					<table class="table table-striped custom-table datatable">
						<thead>
							...
							@endif
								{{--<th>{{ __('sl_no') }}</th>--}}
								<th>{{ __('location_owner_supervisor_name') }}</th>
								<th>{{ __('email') }}</th>
						...
						<tbody>
						@foreach($locationownersupervisor as $val)
							<tr>
								@if($locationownersupervisor->count() > 0)
								<td>
									<label class="form-check form-check-inline">
								...
								<td><img src="{{ url('uploads/profile/' . $val->id. '/locationownersupervisor/'.$val->profile_image) }}" width="50" height="50"></td>
								<td><img src="{{ url('uploads/profile/' . $val->id. '/locationownersupervisor/'. $val->background_image) }}" width="50" height="50"></td>
								...
								@if($val->status ==1)
									<div class="dropdown action-label">
										...
										<div class="dropdown-menu dropdown-menu-right">
											<a class="dropdown-item update-status" href="javascript:void(0);" data-id="{{ $val->id }}" data-url="{{ route('admin.location_owner_supervisor_update_status') }}"><i class="fa-regular fa-circle-dot text-success"></i> {{ __('active') }}</a>
											<a class="dropdown-item update-status" href="javascript:void(0);" data-id="{{ $val->id }}" data-url="{{ route('admin.location_owner_supervisor_update_status') }}"><i class="fa-regular fa-circle-dot text-danger"></i> {{ __('inactive') }}</a>
										</div>
									</div>
								 @else
									<div class="dropdown action-label">
								...
								<td class="text-end">
									<div class="dropdown dropdown-action">
										...
										<div class="dropdown-menu dropdown-menu-right">
											<a class="dropdown-item edit-location-owner-supervisor" href="javascript:void(0);" data-id="{{ $val->id ??''}}" data-url="{{ route('admin.edit-location-owner-supervisor') }}"><i class="fa-solid fa-pencil m-r-5"></i> {{ __('edit') }}</a>
											<a class="dropdown-item delete-location-owner-supervisor-name" href="javascript:void(0);" data-id="{{ $val->id ?? '' }}" data-url="{{ route('admin.getDeletelocationownersupervisor') }}"><i class="fa-regular fa-trash-can m-r-5"></i> {{ __('delete') }}</a>
										</div>
									</div>
								</td>
							</tr>
...
@include('admin.modal.location-owner-supervisor-modal')
@endsection 
@section('scripts')
@include('_includes.footer')
<script src="{{ url('admin-assets/js/page/location_owner_supervisor.js') }}"></script>
...
$(document).ready(function() {
	...
	$(document).on('click',".reset-button", function(){
		window.location.href = "{{ route('admin.location-owner-supervisor')}}";
	});
	
	const translations = {
        addlocation: @json(__('add_location_owner_supervisor')),
    };
	
	$(document).on('click','.add_location_owner_supervisor', function(){
		$('#frmlocationownersupervisor')[0].reset();
		$('#id').val('');
