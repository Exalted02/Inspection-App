	...
	<div class="content container-fluid">
			...
			<div class="row align-items-center">
				...
				<div class="col-md-8 float-end ms-auto">
					<div class="d-flex title-head">
						...
						<a href="#" class="btn add-btn add_checklist" data-bs-toggle="modal" data-bs-target="#add_checklist"><i class="la la-plus-circle"></i> {{ __('add') }}</a>
					</div>
				</div>
			</div>
		</div>
		...
		@csrf
			...
			{{--<div class="col-xl-3">  
					 <div class="input-block">
						 <select name="src_category" class="select select_category" data-url="{{ route('admin.get-subcategory')}}">
							...
							@foreach($categories as $category)
								<option value="{{ $category->id }}" {{ old('src_category', request('src_category')) == (string) $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
							@endforeach
						 </select>
					 </div>
				</div>
				...
				<div class="col-xl-3">  
					 <div class="input-block">
						<input type="text" class="form-control date-range" name="date_range_phone" placeholder="{{ __('from_to_date')}}" id="src_checklist_name_date_range" value="{{ old('date_range_phone', request('date_range_phone')) }}">
					 </div>
				 </div>
				 <div class="col-xl-2">  
					 <div class="input-block">
			...
			<div class="col-lg-6 mb-2">
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						<li><a class="dropdown-item" onclick="change_multi_status('1','Checklist','{{url('admin/change-multi-status')}}')">Active</a></li>
						<li><a class="dropdown-item" onclick="change_multi_status('0','Checklist','{{url('admin/change-multi-status')}}')">Inactive</a></li>
						<li><a class="dropdown-item" onclick="delete_multi_data('Checklist','{{url('admin/delete-multi-data')}}')">Delete</a></li>
					</ul>
				</div>
			</div>
		@endif
								...
								@if($val->status ==1)
									<div class="dropdown action-label">
										...
										<div class="dropdown-menu dropdown-menu-right">
											<a class="dropdown-item update-status" href="javascript:void(0);" data-id="{{ $val->id }}" data-url="{{ route('admin.checklist_update_status') }}"><i class="fa-regular fa-circle-dot text-success"></i> {{ __('active') }}</a>
											<a class="dropdown-item update-status" href="javascript:void(0);" data-id="{{ $val->id }}" data-url="{{ route('admin.checklist_update_status') }}"><i class="fa-regular fa-circle-dot text-danger"></i> {{ __('inactive') }}</a>
										</div>
									</div>
								 @else
									<div class="dropdown action-label">
...
@include('admin.modal.checklist-modal')
@endsection 
@section('scripts')
@include('_includes.footer')
<script src="{{ url('admin-assets/js/page/checklist.js') }}"></script>
...
$(document).ready(function() {
	...
	const translations = {
        addsubcat: @json(__('add_checklist')),
    };
