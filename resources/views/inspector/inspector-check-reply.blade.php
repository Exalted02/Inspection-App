...
@php 
 ...
 if($type == 'checklist')
 {
	...
	/*$corrective_action_final_file_data = App\Models\Task_list_corrective_action_file::where('task_list_corrective_actions_id', $corrective_action_primary_id)->where('status','!=', 1)->get();
	 $corrective_final_action_files = [];
	 if($corrective_action_final_file_data->isNotEmpty())
	 {
		 foreach($corrective_action_final_file_data as $corrective_files)
		 {
			$corrective_final_action_files[] = [
				'url' => url('uploads/corrective_action/' .$corrective_files->file),
			];
...
@endphp
	...
	<div class="container checklist">
		...
		<div style="margin-bottom: 80px;">
			<div class="row">
				<div class="col-md-12">
					<h2 class="owner-checklist-title text-muted">{{ $checklist->name ?? ''}}</h2>
				</div>
			</div>
			@if(!empty($subChecklistName))
			<div class="row">
				<div class="col-md-12">
					<div class="owner-subchecklist-title text-muted">{{ $subChecklistName ?? ''}}</div>
				</div>
			</div>
			@endif
			<div class="row">
				<div class="col-md-12">
					<label>Reason: <span class="text-muted">{{ $rejected_region ?? '' }}</span></label>
				</div>
			</div>
			<div class="row">
				@if(!empty($image_arr))
				...
				@endif
				<div class="col-md-12 text-ia-lo-los d-flex align-items-center flex-wrap">
					<img src="{{ url('front-assets/images/users/4.jpg') }}" class="small-rounded-profile-img mb-0" alt="Profile image">
					<span>By (IA) {{ $userData->get_user->name ?? ''}}</span>
					<span>·</span>
					<span>{{ change_date_format($created_at, 'Y-m-d H:i:s', 'd M Y, h:i A')}}</span>
				</div>
			</div>
			...
			<div class="row IA-IOS-get-reply">
				<div class="col-md-12">
					...
					<div class="mt-1">
						<p class="text-muted mb-0">aaaa bbbb cccc</p>
					</div>
				</div>
			</div>
			...
			<div class="row">
				<div class="col-md-12 mt-1">
					<div class="d-flex flex-wrap gap-3">
						<div class="cheklist-reply-images">
							<img src="{{ url('front-assets/images/users/4.jpg') }}" style="max-width: 150px; height: auto;" target="_blank">
						</div>
						<div class="cheklist-reply-images">
						
						<video src="{{ url('front-assets/images/users/4.jpg') }}" controls style="max-width: 100px; height: auto; border-radius: 10px;" target="_blank"></video>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="row">
				<div class="col-md-6 text-ia-lo-los d-flex align-items-center flex-wrap mt-1">
					<img src="{{ url('front-assets/images/users/4.jpg') }}" class="small-rounded-profile-img mb-0" alt="Profile image">
					<span>By (LO) LO Name </span>
					<span>·</span>
					<span>12 Aug 2025, 08:49 AM</span>
				</div>
			</div>
			<div class="row">
						...
						<div class="experience-box mt-2">
							<ul class="experience-list">
								...
								<li class="pending">
									...
									<div class="experience-content">
										<div class="timeline-content">
											<div class="title-reason mt-1">Pending Approval from IA</div>
										</div>
									</div>
								</li>
							</ul>
			...
			<div class="row IA-IOS-get-reply">
				<div class="col-md-12">
					...
					<div class="mt-1">
						<p class="text-muted mb-0">Replaced the label</p>
					</div>
				</div>
			</div>
								...
								<li class="rejected">
									...
									<div class="experience-content">
										<div class="timeline-content">
											...
											<div class="title-reason mt-1">Replaced with the wrong glove box</div>
											<div class="text-ia-lo-los d-flex align-items-center flex-wrap mt-1">
												<img src="{{ url('front-assets/images/users/4.jpg') }}" class="small-rounded-profile-img mb-0" alt="Profile image">
												<span>By (LO) LO Name} </span>
												<span>·</span>
