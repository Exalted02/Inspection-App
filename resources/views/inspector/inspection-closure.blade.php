...
$locationIds = App\Models\Task_lists::where('inspector_id', $userdata->id)
    ->distinct()
    ->pluck('location_id');
			...
			<section class="custom-padding1">
				<div class="container1">
					...
					@foreach($locationIds as $locations)
						...
						$taskListIds = App\Models\Task_lists::where(['inspector_id'=>auth()->user()->id, 'location_id'=>$locations])->pluck('id');
						//echo "<pre>";print_r($taskListIds);
						...
						<div class="row tab-content mt-2">
							...
							@foreach($correctiveActionPlanData as $result)
									@php 
										...
										$type = '';
										$image = '';
										if($result->subchecklist_id == null)
										{
											$type = 'checklist';
											
											$checklistFile = App\Models\Task_list_checklists::with('get_checklist_files')->where('task_list_id', $result->task_list_id)->where('checklist_id', $result->checklist_id)->first();
										...
										else
										{
											...
											$subChecklistFile = App\Models\Task_list_subchecklists::with('get_subchecklist_files')->where('task_list_id', $result->task_list_id)->where('task_list_checklist_id', $result->checklist_id)->where('subchecklist_id', $result->subchecklist_id)->first();
										...
										$rejectedRegionData = $type == 'checklist'
										? App\Models\Task_list_checklists::where('task_list_id',$result->task_list_id)->where('checklist_id', $result->checklist_id)->first()
										: App\Models\Task_list_subchecklists::where('task_list_id',$result->task_list_id)->where('task_list_checklist_id', $result->checklist_id)->where('subchecklist_id', $result->subchecklist_id)->first();

										if($image != '')
										{
											$images = $type == 'checklist' ?  url('uploads/reject-files/' . $image) :  url('uploads/reject-files/subchecklist/' . $image);
										}
										else{
										...
										{
											$subchecklistData = App\Models\Task_list_subchecklists::where('task_list_id',$result->task_list_id)->where('task_list_checklist_id',$result->checklist_id)->where('subchecklist_id',$result->subchecklist_id)->where('approve',0)->first();
											if($subchecklistData)
											{
