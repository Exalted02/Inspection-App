				...
				<div class="container1">
					<div class="custom-tab" style="margin-bottom: 80px;">
						...
						<div class="row IA-IOS-get-reply">
							...
							<span>By {{ $userData->get_user->name ?? ''}} </span><span>·</span><span>{{ change_date_format($created_at, 'Y-m-d H:i:s', 'd M Y, h:i A')}}</span></div>
						</div>
   ...
   $(document).on('click','.inspector-agree', function(){
	...
	$(document).on('click','.inspector-rejected-submit', function(){
	   ...
	   $.ajax({
			url: URL,
			type: "POST",
			data: {task_id:task_id,checklist_id:checklist_id,subchecklist_id:subchecklist_id,inspector_action:inspector_action,los_action:los_action,first_rejected_reason:first_rejected_reason, _token: csrfToken},
			dataType: 'json',
			success: function(response) {
				if(response.message=='success')
				{
