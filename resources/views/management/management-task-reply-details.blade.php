									...
									<div class="d-flex mb-3 task {{ $class }}">
										...
										<div class="flex-grow-1">
											...
											@endif
												...
												<p class="text-muted mb-0">
												<i class="fa fa-clock">
												{{ Carbon::parse($rejectedRegionData->created_at)->format('d M Y, h:i A') }}<button type="button" class="btn btn-outline-success status-outline-common-btn ml-10px"  style="border-color: #198754; color: #198754;margin-left: 8px;">By {{ $userData->get_user->name ?? ''}}</button></i>
												</p>
												@endif
												<p class="text-muted mb-0">
												<i class="fa fa-map-marker"></i> {{ $location_name ?? ''}}
									...
									@else 
										...
										<div class="flex-grow-1">
											...
											<h6>{{ $checklistName ?? '' }} 
												...
												@if($rejectedRegionData)
												<p class="text-muted mb-0">
												<i class="fa fa-clock">  {{ Carbon::parse($rejectedRegionData->created_at)->format('d M Y, h:i A') }}<button type="button" class="btn btn-outline-success status-outline-common-btn ml-10px"  style="border-color: #198754; color: #198754;margin-left: 8px;">By {{ $userData->get_user->name ?? ''}}</button></i>
												</p>
												@endif
												<p class="text-muted mb-0">
												<i class="fa fa-map-marker"></i> {{ $location_name ?? ''}}
...
$(document ).ready(function() {
   ...
   $(document).on('click','.unCompletetab, .correctiveNeeded, .correctiveChecked, .finalChecked, .approvedFinalChecked', function(){
	   $('#no_record').hide();
		...
		const refreshUrl = "{{ url('management-location-task-details/TASK_ID/ISACTIVE') }}";
		const redirectUrl = refreshUrl.replace('TASK_ID', task_id).replace('ISACTIVE', 0);
		window.location.href = redirectUrl;
	});
